...
class SpotifyConfig:
    ...
    client_secret: str = field(
        default_factory=lambda: os.getenv("SPOTIFY_SECRET_ID") or os.getenv("SPOTIFY_CLIENT_SECRET") or ""
    )
    redirect_uri: str = field(default_factory=lambda: os.getenv("SPOTIFY_REDIRECT_URI", ""))

    # API URLs
    authorize_url: str = "https://accounts.spotify.com/authorize"
    token_url: str = "https://accounts.spotify.com/api/token"
    profile_url: str = "https://api.spotify.com/v1/me"
    ...
    top_tracks_url: str = "https://api.spotify.com/v1/me/top/tracks"
    top_artists_url: str = "https://api.spotify.com/v1/me/top/artists"

    def is_configured(self) -> bool:
        """Check if the shared Spotify OAuth application is configured."""
        return bool(self.client_id and self.client_secret)


@dataclass(frozen=True)
class AppConfig:
    """Application settings for the public auth flow."""

    base_url: str = field(default_factory=lambda: os.getenv("APP_BASE_URL", "http://127.0.0.1:5000"))
    secret_key: str = field(default_factory=lambda: os.getenv("SECRET_KEY", "dev-secret-change-me"))
    token_db_path: str = field(default_factory=lambda: os.getenv("TOKEN_DB_PATH", "/tmp/spotibadge.sqlite3"))
    ...
    default_widget_name: str = "Spotify"

    def use_redis_store(self) -> bool:
        """Return true when Redis REST storage is configured."""
        return bool(self.redis_rest_url and self.redis_rest_token)

    def callback_url(self) -> str:
        """Return the Spotify callback URL."""
        if spotify_config.redirect_uri:
            return spotify_config.redirect_uri
